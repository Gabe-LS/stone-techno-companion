...
- **Teardown is atomic and single-sourced**: `delete_meetup` deletes attendee/meetup rows *then* `delete_room` last (one commit); `delete_user` and `admin_delete_meetup` both funnel through it; `purge_expired_meetups` deletes by captured ids, not a re-query by timestamp. Creator-only **cancel** is `DELETE /meetups/{id}` (gated on `creator_id`), which broadcasts `meetup_expired` and evicts `manager.rooms`/`_room_meta`.
...
**Attendance = membership:** join/leave/create write `room_memberships` for meetup rooms (not just `meetup_attendees`), so meetup rooms are first-class in the shared membership/unread/reachable-count/push machinery. Opening a meetup room shows a **details banner** (`renderMeetupDetails`) with when/where/note/photo/Open-in-Maps and the real attendee count; the banner and the attendee-gated `GET /meetups/{id}` are how members see location/photo (never the room-wide broadcast). Clicking the invite card opens the meetup room. The creator can cancel from the invite card, the room banner, or by re-tapping the selected sidebar row; deleting the invite card (creator, within the 120 s message-delete window) also cancels the meetup + room.
