...
**Location & photo capture:** the create modal offers GPS, an interactive **Leaflet map picker** with a draggable pin (`openMeetupMapPicker`; MapTiler tiles when `MAPTILER_KEY` is set, else OSM; Leaflet lazy-loaded from CDN), and an optional **meeting-point photo** (`meetups.photo_url`, migration-added). The photo is uploaded via the normal `/upload/image` pipeline, then run through OpenAI moderation in the create handler (`validate_meetup_photo` in `chat_ws.py`, shared by WS + REST); its URL is validated against the upload allowlist, attendee-gated in `_shape_meetup`, and unlinked on meetup delete/expiry. Stored GPS is still rounded to ~11 m.
