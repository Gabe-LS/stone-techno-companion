...
- **Artist schedule notes** (line-up list view only): floor + time on every card, "Also" cross-references for multi-slot artists. The timetable surfaces floor info only via the popup and each card's `data-floor` attribute.
