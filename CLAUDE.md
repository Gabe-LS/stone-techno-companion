...
- **No em dashes as punctuation** in any NEW text: UI copy, docs, comments, commit messages, assistant replies. Use commas, colons, periods, or parentheses instead. (Pre-existing text is left as is; do not churn old files to fix it.)
...
stage_details     — stage_id, label, value, position (RESERVED: schema exists but no code inserts or reads it yet)
...
| `server/static/pages/transport.html` | Tram 107 / NE2 realtime departure board SPA (see "Public transport page") |
| `pipeline/transport/capture-api.mjs` | Regenerates the transport schedule JSON from the VRR EFA API (Node 18+) |
...
| `tests/test_chat_api.py` | 70 tests — REST endpoints, auth (incl. email code login), rooms, meetups (incl. cancel-deletes-invite), DMs, admin |
...
| `tests/verify_push_both.py` | Standalone push check — after enabling notifications on both lineup and chat in one Chromium profile, fires a real WebPush at each stored subscription and asserts both surfaces share one LIVE endpoint (see push "Hard-won invariants") |
| `tests/test_transport.py` | 9 tests: transport departures proxy (mocked upstream: filtering, cache, rate limits, bounds) |
...
- **Mobile**: HTML `<table>`, `table-layout: fixed`, dynamic `--row-h` (10px or 14px based on artist density). The document does NOT scroll: `.view-timetable body { overflow: hidden }` fixes the chrome physically in place (`position: static` on the bar/title/day-tabs, since sticky is meaningless without document scroll). The only scrolling surface is the active panel's `.tt-v-scroll` (both axes); the floor-name `<thead>` and the time column are sticky against THAT scroller. The page title compacts via a CSS scroll-driven animation (`animation-timeline: scroll()` + `timeline-scope` tied to the panel scroller) with a static-title fallback where scroll timelines are unsupported
...
- PWA meta tags: `mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style`, `apple-mobile-web-app-title`, `theme-color`
- Modal overlays close on backdrop click only when the press STARTED on the backdrop (pointerdown tracking): releasing a text selection or image pan over the overlay must not close the dialog. Lineup: guard in the central `.modal-overlay` handler; chat: `_bindBackdropClose` helper (drag-distance gate in the image viewer, where click-on-image closes by design)
...
- Every artist-facing href passes a render-layer URL scheme allowlist, so anything that isn't `http(s)` (or `mailto` for social links) renders as `#`: `_safe_href` (Python) + `_safeHref` (emitted popup JS) for artist social links, and an inline `^https?://` guard on bio "Sets" video links. Chat's link-preview cards apply the same guard client-side (`_safeHref` in `chat.html`). This is defense-in-depth at the last output stage even though scraped links are already http(s)-validated — it blocks a `javascript:` URL injected via a hand-edited `overrides.toml`.

## Page Load Flash Prevention

All pages use `body{opacity:0}` in an inline `<style>` in `<head>` (first element) to prevent content from flashing before JS initialization completes. The lineup page reveals via `Promise.race([initPromise, 5s timeout])`: opacity flips to 1 when the async init IIFE settles (success or failure) OR after a 5-second failsafe, so a hung `fetch()` can never leave the page blank. The deep-link action runner polls this opacity flag as its "init done" gate (its own giveup is 6s, deliberately past the reveal timeout).

The lineup/timetable page has two more head scripts (before body): the FIRST handles installed-app resume: on a home-screen launch at `/` (`navigator.standalone` or `display-mode: standalone`) with `localStorage.last_section === 'chat'` it `location.replace`s to `/chat`; in a normal browser tab `/` never goes to chat. The SECOND sets `document.documentElement.className='view-list'` or `'view-timetable'` based on the URL/localStorage (`stc_view`, the last lineup view SEEN), and rewrites `/` to the canonical `/line-up` or `/timetable` via `history.replaceState`. CSS rules keyed to these classes control which view, buttons, and menu items are visible — so the correct view state is applied before any body content is parsed. The `switchView()` function updates this class when switching views at runtime.
...
**Public transport page** (`/transport`): realtime departure board for Tram 107 / NE2 from Zollverein to Essen Hbf (how festival-goers get home). Static schedule in `server/static/timetable-transport.json` (served with no-cache via an explicit route), regenerated by `node pipeline/transport/capture-api.mjs` (Node 18+, edit its `dates` array for new events; output lands in `server/static/`, deploys via git pull). Realtime layer: the page polls `GET /api/transport/departures?date=DD.MM.YYYY&time=HH:MM` every 30 s when viewing today; the endpoint proxies the VRR EFA API (`efa.vrr.de`, no CORS upstream), with the stop id PINNED server-side (not client-controlled, so it cannot be abused as an open proxy), a shared 55 s cache per (date, minute) bucket so many concurrent clients collapse to ~1 upstream call/min, per-IP rate limiting, and stale-cache fallback on upstream errors. Walk-time feature: explicit "Locate me" button; GPS goes to `GET /api/transport/walk?lat=&lng=` which proxies OSRM foot routing server-side (the FOSSGIS routed-foot instance at routing.openstreetmap.de; the project-osrm.org demo only hosts the car profile and silently returns driving routes) (third party sees only the VPS IP; coordinates are transient, never stored or logged; bounded to the Essen area). Linked from the lineup command bar + hamburger ("Transport") and the chat mobile menu ("Public transport"). Tests: `tests/test_transport.py` (mocked upstream: filtering, cache, rate limits, bounds).
...
email_tokens       — token, email, provider_id, fingerprint, code, code_attempts, expires_at (DB-backed, survives restart)
...
chat_settings      — key, value (app-level config: room_sort, msg_char_limit, dm_ttl_minutes, room_ttl_minutes, meetup_ttl_minutes, meetup_bbox, meetup_bbox_expand_pct)
room_memberships   — user_id + room_id (PK), joined_at, last_read_at (tracks joined rooms + unread)
messages           — id, room_id, user_id, type, content, link_preview, reply_to_id, media_url, moderation_status, expires_at, created_at
message_reactions  — message_id + user_id + emoji (PK), created_at, CASCADE on message delete
meetups            — id, creator_id, stage_id, title, location_lat, location_lng, location_label, meetup_time, note, photo_url, created_at, expires_at
...
Two passwordless providers: Google OAuth and Email magic link (via Maileroo, 3,000/mo free). The magic-link email also carries a **6-digit sign-in code** (`POST /chat/api/login/code`): iOS home-screen web apps have cookie storage partitioned from Safari and emailed links always open in the browser, so the link can never authenticate the PWA; typing the code in the app creates the session in the requesting context. The code is stored hashed next to the link token (same row, same 15-min TTL, single use), and is burned after 5 wrong attempts. Disposable domains blocked via 7,860-domain blocklist (`chat/disposable_domains.txt`). Email validation via `email-validator` library (RFC 5322 + DNS MX check). Ban enforcement is provider-based and covers every linked provider of a user (Google + email); the `bans.device_fingerprint` column exists and the matching logic works, but clients do not currently submit a fingerprint, so it is unused (bans by a **new** account are possible — mitigated by moderation, not fingerprinting). Re-linking a fresh provider to an existing banned account is blocked: `_authenticate` calls `is_user_banned(user_id)` (which checks bans across the frozen identity AND every `user_providers` row) for any already-existing user before issuing a session, and the OAuth paths refuse to link a new provider onto a banned account — so a banned user can't regain their original account by signing in through another provider on the same verified email. The email-based ban check in Google OAuth runs regardless of `email_verified` (only the account-linking step itself requires a verified email), closing an evasion path via an unverified Google email. Magic-link email is also rate-limited per destination address (3 / hour) on top of the per-IP limit, to block mail-bombing a victim. A banned email hitting `/chat/api/login` gets a generic "This account cannot sign in." 403 (no ban reason, no registration-status leak); the WS `banned` event also hides admin-authored ban reasons, sending a generic "You have been banned." instead. Session cookies (non-httpOnly for WS access, Secure in production, SameSite=Strict in production / Lax in dev, path=/). Each login mints its own session row/token, so logout is **per-device**: `/chat/api/logout` deletes only the calling device's session token and closes only that device's WebSocket(s) — other devices keep their own sessions and stay connected. The socket is targeted via `ConnectionManager.conn_token` (conn_id → session token, populated in `connect()`); `close_session(user_id, token)` closes only connections whose token matches. Email tokens stored in DB (not memory) — survive server restarts. **Rate limits** (per client IP, in-memory, self-pruning): magic-link `/chat/api/login` 5 / 15 min; the OAuth + verify endpoints (`_check_auth_rate`) 120 / 5 min — high because a festival venue shares a public IP and magic-link tokens are 256-bit (`secrets.token_urlsafe(32)`) / OAuth is Google-validated, so brute force is not the threat this guards.
...
- **Media file cleanup for E2EE DMs**: the file URL is encrypted inside the envelope, so the server can't find the file to garbage-collect it by parsing message content. The client sends the plaintext URL in a top-level `media_url` field, stored in `messages.media_url`; the TTL purge, manual delete, meetup expiry, and admin room-delete all read that column to unlink the served file (and its `_mod*.webp` copies). Without this, encrypted image/video DMs orphaned their files on disk forever. **All** unlink paths go through `_unlink_media_if_orphaned` (or replicate its check inline, WS `delete_message`) — a file is deleted only when no other live message still references the same `media_url`, so a crafted `media_url` pointing at another user's file can't delete it on purge/ban/expiry. This includes the WS moderation-reject and pre-broadcast ban/mute-recheck cleanups, which delete the message row first and then orphan-check the unlink (they used to unlink unconditionally; fixed July 2026).
...
- **No history sync**: a newly registered device cannot decrypt messages sent before it existed; the DM message TTL (24h default, `dm_ttl_minutes`) bounds how long that gap is visible.
- **Specs**: `docs/e2ee-v1.md` (v1 design + server adaptations — v2 inherits its trust model and fallback rules) and `docs/e2ee-multidevice.md` (v2 multi-device design, current).
...
Real-world meetups: a user creates one (title, date/time, location (required — GPS pin via map picker or text label), optional note up to 80 chars, optional meeting-point photo), which spins up a dedicated `type='meetup'` chat room (room id == meetup id, joined by UUID convention — no FK), an attendee/RSVP list, and (when tied to a `stage_id`) a `meetup_invite` card broadcast into that room. Auto-expires `meetup_ttl_minutes` after the meetup time.

**Security/privacy model (hardened July 2026):**
- **Location + attendee identity are attendee-only.** `GET /meetups` and `GET /meetups/{id}` route through `_shape_meetup(...)` in `chat_api.py`: non-attendees get only `id/title/meetup_time/stage_id/attendee_count/is_going/is_creator/expires_at`; only attendees (rows in `meetup_attendees`) get `location_lat/lng`, `location_label`, `note`, and the named `attendees` list. The `meetup_created` WS broadcast carries only `id/title/meetup_time/stage_id` — never coordinates. When adding any new meetup read path, gate it the same way (mirror `_shape_meetup`). The `meetup_invite` card intentionally shows `title`/`label`/`note` room-wide (textual, creator-chosen) but NEVER precise GPS; the "Open in Maps" link is injected client-side only from the attendee-gated `GET /meetups/{id}` response (`loadMeetupJoinState`).
- **Creation is fully gated** (both the WS `create_meetup` event AND `POST /meetups` — keep them in lockstep): `check_ban_mute` (muted/banned users blocked), word-filter on `title`+`note`+`label`, and `meetup_time` must be timezone-aware, in the future, and ≤ 30 days out. GPS is rounded to 4 decimals (~11 m) and range-validated in `create_meetup` (`_sanitize_coord`).
...
- **POIs:** from the MapTiler dataset (`/chat/api/pois`, see `MAPTILER_DATASET_ID`), rendered as HTML markers — dot and name-label are **separate markers** so z-order is labels > red drag-pin > dots; tapping a POI drops the pin on it. Category → colour via `POI_COLORS` (with sub-type overrides); unknown categories get a stable palette colour from `POI_PALETTE`.
...
- **Label collision avoidance:** on map load, POIs are projected to screen coordinates at the label-visible zoom level. For each POI, if another POI sits in the label zone to its right (within 100px × 20px), the label flips to anchor `'right'` (placed to the left). Hover on a dot shows its label at any zoom.
- **Considered but deferred: SDF sprite layer** — would eliminate the sub-pixel HTML-marker jitter during pinch-zoom and give GPU-rendered icons with built-in collision avoidance, but adds complexity (canvas-rendered SDF registration, `map.on('click', layer)` wiring) for a marginal visual improvement with ~20 POIs. Current HTML markers are adequate.
- **Framing is POI-driven:** `/config` returns `meetup_bounds` = the POI extent made **square (in metres) × (1 + `meetup_bbox_expand_pct`)** (chat_settings, default **15%**; empty `meetup_bbox` = auto, or set it to `"w,s,e,n"` to override). The client fits that square inside the container (`fitBoundsOptions`), then locks **min-zoom to the fitted zoom and `maxBounds` to the fitted view** — so the POIs+expand are **always fully visible on any aspect ratio** and default-zoom == min-zoom (zoom in only). Custom attribution: a fixed "i" that opens a full-width credit box to its left (credits Geobasis NRW, OpenFreeMap, OpenStreetMap, MapLibre); any map click closes it.
...
Reports from E2EE DMs carry reporter-provided plaintext the server never independently verified — flagged `unverified` in the `reports` table, shown with a warning banner in the admin UI alongside the reporter/reported user history. **Retention** (deliberate, see `purge_old_reports`): reviewed reports (actioned/dismissed) are kept 30 days; pending reports are kept until an admin reviews them. This is intentionally longer than the 24h DM TTL because the snapshot is the only moderation evidence for an E2EE DM, so reporter-provided plaintext can persist past the DM's own TTL as an evidence hold.
...
- **Meetup creation**: modal with title, date + hour/minute selects (15-min intervals), location (required — text input + icon-only map-picker button), note (80-char limit with red-border visual feedback matching message input, hard cap at 100), and meeting-point photo (21:9 clickable drop zone).
...
- **Signed-out chrome**: the sign-in and profile-setup screens (shared `_authChromeTop`/`_authChromeMenu` helpers) show the mobile chat header + hamburger on mobile, and on desktop hide it and render the shared cmd-bar (Line-up | Timetable | Chat | Transport) with the exact centered-column geometry of the other pages (`auth-mode` class on `#app`)
- **URL structure**: `/line-up` (lineup), `/timetable` (timetable), `/chat` (main chat), `/chat/r/{id}` (room), `/chat/d/{user}` (DM), `/chat/m/{id}` (meetup), `/chat/v/{token}` (verify email), `/chat/msg/{id}` (message permalink), `/chat/admin` (admin). API under `/chat/api/`. `/` in a normal browser tab always resolves to the last used LINEUP view (`stc_view`: `/line-up` or `/timetable`, never chat/transport); only an installed home-screen app (standalone mode) resumes into `/chat` when chat was the last visited section (`last_section` flag, stamped by chat, reclaimed by any lineup visit). One shared manifest (`start_url: "/"`) for the whole app: a separate chat manifest was tried and reverted because a second app identity means a second iOS storage partition (own sign-in, own push subscription).
...
- **Debug**: 300+ `dbg()` calls with timecodes across all functions, `verify()` checks DOM state

### Chat Tests

297 tests total — 243 core (`python -m pytest tests/ -v`: 234 chat + 9 transport) + 54 notification (Playwright, run separately):
...
- `test_chat_api.py` (70) — REST endpoints, auth (incl. email code login), rooms, meetups (incl. cancel-deletes-invite), DMs, admin
