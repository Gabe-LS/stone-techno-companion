...
| `tests/test_chat_api.py` | 57 tests — REST endpoints, auth, rooms, meetups (incl. cancel-deletes-invite), DMs, admin |
...
- **`stage_id` is validated before the invite card is posted**: it must be an existing, non-read-only `stage`/`general` room (never `dm`/`meetup`). The same `valid_target` flag also gates the `meetup_created` broadcast room, so an unauthorized `stage_id` falls back to the main room. This blocks message injection into read-only/announcement rooms and E2EE-bypass into DMs.
...
221 tests total: `python -m pytest tests/ -v`
...
- `test_chat_api.py` (57) — REST endpoints, auth, rooms, meetups (incl. cancel-deletes-invite), DMs, admin
