...
`pipeline/scraper/render.py` bakes directly into `lineup.html` today. It
does not cover the favorites/schedule write API already implemented in
`server/api.py` (`/api/session/*`), except where this design must match its
...
`docs/parity/timetable.md`, `pipeline/scraper/timetable_json.py`,
`pipeline/scraper/db.py`, `pipeline/scraper/render.py`, and a local read of
`pipeline/lineup.db` (row counts and byte sizes cited below are measured
from that file and from `pipeline/output/` on 2026-07-13, not estimated).
...
`server/api.py` against `hearts.db`. This API only serves the festival
...
`pipeline/scraper/timetable_json.py` already computes, not a new id scheme.
...
FastAPI app (`server/api.py`), which already serves the favorites/schedule
...
- Both are served as plain static files (today via the pipeline's
  `--deploy` rsync into `server/static/`; see section 3 for how this
...
- **`timetable.json` (generated by `pipeline/scraper/timetable_json.py`):
  ...
  (`server/api.py`'s `_push_notification_scheduler`, matching due slots
...
The existing favorites/schedule API (`server/api.py`, `hearts.db`) is
...
must equal, byte for byte, the ids `server/api.py` already validates and
stores.
...
- **Format constraint, not just value**: `server/api.py`'s `UUID_RE` is
...
**Pagination.** Recommendation: none. Measured from `pipeline/lineup.db`
