...
- **Rule**: `slot_uuid()` in `services/data/scraper/timetable_json.py` (also imported by `services/data/scraper/render.py`) is the single source of truth for a set's identity, used for saved schedules, push dedup, and ICS export. It is collision-aware: an artist playing two sets on the same floor within one date+period no longer collapses to one id, and existing ids are preserved (the earliest slot keeps the historical id, only the extra one is disambiguated).
...
- **Field data caveat** (retrospective 2026-07, see `docs/retrospective-2026-07.md`): the production sample after Stone Techno 2026 showed ZERO endpoint overlap between the two tables (9 lineup, 4 chat subscriptions). Too small to prove or disprove the invariant (users may simply have enabled only one surface each), but it means production data does NOT currently demonstrate the shared-endpoint behavior. Any push-affecting change must therefore rely on `verify_push_both.py` under a controlled dual-enable, never on observed production overlap.
