...
- **Client**: the validation moves to render time — after decrypting an image/video message, validate the inner URL against the same pattern (`/chat/api/uploads/` + `[a-f0-9]{32}.(webp|mp4)`) before inserting it into the DOM. On mismatch, render the `[Encrypted message]` fallback. This matters: the decrypted URL is attacker-controlled by the peer, and it must never land in a `src` attribute unvalidated. (The uploads endpoint's own filename allowlist + `nosniff`/CSP headers are a second layer, not a substitute)
